...
export const isAuthenticated: RequestHandler = async (req, res, next) => {
  ...
  if (now <= user.expires_at) {
    ...
    try {
      ...
      if (userData && userData.status === 'banned') {
        return res.status(403).json({ message: "Account has been banned", banned: true });
      }
    } catch (error) {
      // If storage check fails, continue anyway to avoid breaking auth
  ...
  try {
    ...
    // Check if user is banned after token refresh
    try {
      const { storage } = await import('./storage.js');
      const userData = await storage.getUser(user.claims.sub);
      if (userData && userData.status === 'banned') {
